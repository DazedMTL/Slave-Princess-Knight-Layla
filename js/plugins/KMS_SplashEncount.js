...
(function () {
  ...
  var Params = {};
  Params.splitNum = {};
  Params.splitNum.x = Number(pluginParams["Split X"] || 8);
  Params.splitNum.y = Number(pluginParams["Split Y"] || 6);
  ...
  Game_SplashScreen.prototype.initialize = function () {
    ...
    this._canvas3d = new THREE.WebGLRenderer({ antialias: false, alpha: true });
...
*/
    ...
    this._splashTexture = new THREE.Texture(Bitmap.snap3(stage));
    this._splashTexture.minFilter = THREE.LinearFilter;
    ...
    this._splashObjs = this.createSplashMeshes(
      ...
      { x: Graphics.width - 300, y: Graphics.height - 200 },
      Params.splitNum
    );
    ...
    var mesh = new THREE.Mesh(geometry, this._splashMaterial);
    this._scene3d.add(mesh);
    this._splash = mesh;

    // 3D 空間を 2D テクスチャにマッピング
    this._texture3d = PIXI.Texture.from(this._canvas3d.domElement);
   ...
   * 破片メッシュの作成
   */
  Game_SplashScreen.prototype.createSplashMeshes = function (
    material,
    wholeSize,
    splitNum
  ) {
    ...
    for (var i = 0; i < splitNum.x * splitNum.y; ++i) {
      ...
      for (var j = 0; j < 2; ++j) {
        ...
        var uvd = {
          u: splitSize.x / wholeSize.x,
          v: splitSize.y / wholeSize.y,
        };
        if (j === 0) {
          ...
          geo.vertices[1] = new THREE.Vector3(
            pos.x + splitSize.x,
            pos.y + splitSize.y,
            0
          );
        ...
        mesh.delta_mov = new THREE.Vector3(
          randRot(),
          randRot(true),
          randRot(true)
        );
  ...
  Game_SplashScreen.prototype.updateAnimation = function (duration) {
    ...
    this.updateObjects();
    if (duration >= 20) {
      this._splashMaterial.opacity -= 0.08;
    }
  };

  Game_SplashScreen.prototype.updateObjects = function () {
    if (this._splash) {
      this._scene3d.remove(this._splash);
      //this._splash.quaternion.multiply(this._splash.delta_quat);
    }
    var geometry = new THREE.Geometry();
  ...
  const _Scene_Map_prototype_destroy = Scene_Map.prototype.destroy;
  Scene_Map.prototype.destroy = function () {
    this.removeChild($gameTemp.splashScreen);
    _Scene_Map_prototype_destroy.call(this);
  };
  ...
  var _KMS_SplashEncount_Scene_Map_updateEncounterEffect =
    Scene_Map.prototype.updateEncounterEffect;
  Scene_Map.prototype.updateEncounterEffect = function () {
    if (ConfigManager.performance) {
      _KMS_SplashEncount_Scene_Map_updateEncounterEffect.call(this);
      return;
    }
  ...
  Scene_Battle.prototype.updateEncounterEffect = function () {
    ...
    _flag = !_flag;
    if (_flag) {
      $gameTemp.splashScreen.updateAnimation(n - a);
      $gameTemp.splashScreen.render3d();
    }
